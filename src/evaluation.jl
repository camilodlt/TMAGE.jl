...
function evaluate(
        tpg_program::TPGProgram, input::Any, hashed_input::UInt64, cache::TPGEvaluationCache,
        shared_inputs::SharedInput, meta_library::MetaLibrary, model_architecture::modelArchitecture,
        mode = "notwarmup"
    )
    # If cache is disabled, or mode is NoCache, directly evaluate
    @timeit_debug to "Eval Prog : cache disabled" begin
    ...
    @timeit_debug to "Eval Prog : Hash" input_hash = hashed_input # Hash the input for caching
    @timeit_debug to "Eval Prog : Cache lookup" cached_value = get_cached_value(cache, tpg_program.id, input_hash)
    ...
    if cached_value !== nothing
        ...
        # println("Did not have value for $(tpg_program.id)")
        # @info "No cached value"
        @timeit_debug to "Eval Prog : No cache" begin
            if tpg_program.program === nothing
                tpg_program.program = UTCGP.decode_with_output_nodes(tpg_program.genome, meta_library, model_architecture, shared_inputs).programs[1]
            end
            UTCGP.replace_shared_inputs!(tpg_program.program.program_inputs, input)
...
function evaluate(
        tpg_program::TPGProgram, input::Any, cache::TPGEvaluationCache,
        shared_inputs::SharedInput, meta_library::MetaLibrary, model_architecture::modelArchitecture,
        mode = "notwarmup"
    )
    @timeit_debug  to "Hash Input" tmp = hash(input)
    return evaluate(
        tpg_program, input, tmp, cache,
        shared_inputs, meta_library, model_architecture,
        mode
    )
end
...
"""
function evaluate(
        tpg_team::TPGTeam, input::Any, hashed_input::UInt64, cache::TPGEvaluationCache,
        shared_inputs::SharedInput, meta_library::MetaLibrary, model_architecture::modelArchitecture
    )
    bids = Dict{ProgramID, Any}()
    @timeit_debug to "Eval loop All Programs" begin
        for program in tpg_team.programs
            bids[program.id] = evaluate(program, input, hashed_input, cache, shared_inputs, meta_library, model_architecture)
        end
    end
...
function evaluate(
        tpg_team::TPGTeam, input::Any, cache::TPGEvaluationCache,
        shared_inputs::SharedInput, meta_library::MetaLibrary, model_architecture::modelArchitecture
    )

    @timeit_debug  to "Hash Input" tmp = hash(input)
    return evaluate(
        tpg_team, input, tmp, cache,
        shared_inputs, meta_library, model_architecture
    )
end
...
function evaluate(
        tpg::TangledProgramGraph, root_team_id::TeamID, input::Any, hashed_input::UInt64, shared_inputs::SharedInput, meta_library::MetaLibrary, model_architecture::modelArchitecture;
        cache::TPGEvaluationCache
    )
    return @timeit_debug  to "EVAL TPG" begin
        ...
        @timeit_debug  to "Traverse TPG" begin
            while current_team_id !== nothing
                ...
                @timeit_debug to "Eval team" team_output, next_team_id, winning_program_id = evaluate(current_team, input, hashed_input, cache, shared_inputs, meta_library, model_architecture)
